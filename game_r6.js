...
function onSetup() {
    ...
    // issue : refactor to use prototyped player objects
    // issue : crockford seems against new Object as in makeObject for using new, not sure this is just being overly picky, maybe just {} is good enough.
    player = makeObject();
    player.speed = 10;
    player.action = 0; 
    player.height = 90;
    ...
    process = function () {
        this.speed = 10;	// on screen speed
        this.action = 0;	
        this.height = 90;	// display height of the character
        this.markTime = currentTime();	// time marker for decision making

        //pos : XXpos_object,	// current planar pose in the game
        //vel : XXvel_object,	// current 2D command velocity in the game
        this.pos = Object.beget(position);
        this.vel = Object.beget(velocity);

        this.sprite_index = 0; // index for drawing sprite (associated with specific sprite sheet)
    ...
    if (!game.state) {
        for(i=0;i<npc.length;i++)  {
            if (currentTime() > npc[i].markTime + 1) { 
                ...
                //npc[i].disc.action = 1;
                //npc[i].disc.action = Math.round((Math.random()*100)-92);
                npc[i].disc.action = Math.round((Math.random()*10)-2);
...
function animate() {
   ...
   if (!game.state) {
        ...
        var temp_player_hit = false;
        for(i=0;i<npc.length;i++)  {
            temp_distance = Math.sqrt((npc[i].disc.pos.x - player.pos.x)*(npc[i].disc.pos.x - player.pos.x)+(npc[i].disc.pos.y - player.pos.y)*(npc[i].disc.pos.y - player.pos.y)); 

            if ((temp_distance < 50)&&(npc[i].disc.state == 1)) {
                temp_player_hit = true;
            }
        }

        if (temp_player_hit) {
            player.pos.x = screenWidth/4;
       ...
       // animate disc
        ...
        for(i=0;i<npc.length;i++)  {
            ...
            temp_distance = Math.sqrt((npc[i].pos.x - disc.pos.x)*(npc[i].pos.x - disc.pos.x)+(npc[i].pos.y - disc.pos.y)*(npc[i].pos.y - disc.pos.y)); 

            if ((temp_distance < 50)&&(disc.state == 1)) {
                npc[i].pos.x = screenWidth/4;
                npc[i].pos.y = screenHeight/4;
                game.score += 50;
            }
            else {
